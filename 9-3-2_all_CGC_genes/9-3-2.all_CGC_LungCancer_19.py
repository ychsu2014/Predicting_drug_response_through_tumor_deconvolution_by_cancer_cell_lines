...
out_cancer_type = "LungCancer"

in_mut = "/home/CBBI/hsuy1/projects/drugResponse/data/ICIBM_2023/TCGA/processed_data/mutation/TCGA_all_mutations_simplifiedColumns_hg19ConvertedToGRCh38_filtered_7781_CGC_matrix.txt"
...
in_drug_via = "/home/CBBI/hsuy1/projects/drugResponse/data/ICIBM_2023/TCGA/processed_data/drug_score/exponential_log/lung_cancer_all_oncoKB_19/"
###### modified for parallele processing
out_file = "/home/CBBI/hsuy1/projects/drugResponse/data/ICIBM_2023/TCGA/processed_data/drug_score/exponential_log/lung_cancer_all_oncoKB_19/mut_grouped_drug_prediction_t_test_p_values_CGC_" + out_cancer_type + "_v2.txt"
...
mut_df = pd.read_csv(in_mut, sep = "\t")
mut_df.set_index("sample", inplace = True)
cancer_mut_df = mut_df.loc[cancer_all_sID_list]
CGC_gene_list = cancer_mut_df.columns
cancer_mut_sID_dict = {}
for agene in CGC_gene_list:
    temp_df = cancer_mut_df[agene]
    mut_sID_list = list(temp_df[temp_df == 1].index)
    cancer_mut_sID_dict[agene] = mut_sID_list

print("Number of mutation: " + str(len(CGC_gene_list)))
...
for agene in cancer_mut_sID_dict.keys():
    ...
    if len(mut_sID_list) > 1 and len(control_sID_list) > 1:
        for drug in all_drug_list:
            ...
            all_data_dict[(agene, drug)] = temp_dict
...
count = 0
for (gene, drug) in all_data_dict.keys():
    count += 1
    if count % 100 == 0:
        print(count)
    grouped_via_dict = all_data_dict[(gene, drug)]
    if 1 in grouped_via_dict.keys() and 0 in grouped_via_dict.keys():
        ...
        if len(mut_via_list) > 1 and len(wt_via_list) > 1:
            ...
            fout.write("\t".join([out_cancer_type, gene, drug, str(mut_num), str(wt_num), str(mut_mean), str(wt_mean), str(mut_wt_diff), str(p_value)]) + "\n")
fout.close()
