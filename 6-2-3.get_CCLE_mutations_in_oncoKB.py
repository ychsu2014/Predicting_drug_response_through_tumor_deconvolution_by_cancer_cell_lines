...
##### two variables are changed for CCLE datast
in_keep = "/home/CBBI/hsuy1/projects/drugResponse/data/ICIBM_2023/oncoKB/processed_data/oncokb_biomarker_drug_associations_autoCheck_included_final.txt"
in_drop = "/home/CBBI/hsuy1/projects/drugResponse/data/ICIBM_2023/oncoKB/processed_data/oncokb_biomarker_drug_associations_autoCheck_excluded_final.txt"
...
def writeToFile(inFile, outFile, outDrop, outExclude, inOtherKeep = other_keep, inAllaaKeep = all_aa_keep, inWildKeep = wild_keep, inKeepMutLineDict = keep_mut_line_dict, inOtherDrop = other_drop, inAllaaDrop = all_aa_drop, inDropMutLineDict = drop_mut_line_dict, inOncoKeep = onco_keep):
    ...
    for line in lines:
        line = line.replace(",", "\t")  ##### changed for CCLE dataset
        cols = line.strip("\n").split("\t")  ##### changed for CCLE dataset
        gene = cols[0]  ##### changed for CCLE dataset
        alt = cols[18].replace("p.", "")  ##### changed for CCLE dataset
...
out_file = in_file.replace(".txt", "_in_oncoKB.txt")
out_drop = in_file.replace(".txt", "_oncoKB_dropped.txt")
out_exclude = in_file.replace(".txt", "_oncoKB_excluded.txt")
