in_path = "/home/CBBI/hsuy1/projects/drugResponse/data/ICIBM_2023/TCGA/processed_data/mutation/"
in_file = in_path + "TCGA_all_mutations_simplifiedColumns_hg19ConvertedToGRCh38_filtered_7781_in_oncoKB_final.txt"
out_file = in_path + "TCGA_all_mutations_in_oncoKB_uni_var.txt"
out_file2 = in_path + "TCGA_all_mutations_in_oncoKB_uni_var_stat.txt"
out_file3 = in_path + "TCGA_all_mutations_in_oncoKB_uni_entry_with_duplicated_vars.txt"
...
for line in lines:
    cols = line.strip("\n").split("\t")
    scRNA_cancer_type = cols[16]
    var = "\t".join(cols[1:len(cols)-5]) + "\n"
    uni_var_dict = parseToDict(uni_var_dict, scRNA_cancer_type, var)
    entry = "\t".join(cols[1:]) + "\n"
    uni_entry_dict = parseToDict(uni_entry_dict, scRNA_cancer_type, entry)
